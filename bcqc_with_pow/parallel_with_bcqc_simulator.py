...
Program title: Parallel Manufacturing with BCQC
...
is conducted in parallel sequence, which means each product can randomly choose the next step based on the current
...
scenario_number = input("Enter scenario number [only 5/6/7]")
...
# todo: create array storing status of each secondary validator for each step
...
def secondary_validation(thr):
    ...
    for i in range(len(quality_array)):
        ...
        # print("SEC VALIDATION ARRAY", quay)
...
def sec_validation_correctness():
    ...
    """
    # Each validator has threshold% accuracy of correct secondary validation
    ...
    for xx in range(0, total_number_of_parts):
        for yy in range(0, total_number_of_steps):
            for quality in range(0, len(correctness)):
                ...
                if correctness[quality] > threshold_array[xx][yy]:
                    count += 1
            # print("Count ", count)
            if count > 7:
...
# Finding actual secondary validation status of each process
get_threshold()
...
while parts < total_number_of_parts:
    ...
    while step_number < total_number_of_steps:
        ...
        write_to_excel(step_number, parts, variables.loc[step_number, 'Production_Time'],
                       ...
                       index4,
                       uid4, time_stamp, threshold, step_status, quality_array, sec_validation_status,
